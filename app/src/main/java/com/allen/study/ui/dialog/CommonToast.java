...
import com.allen.study.utli.TLog;
...
public class CommonToast {
	...
	public void show() {
		final ViewGroup content = (ViewGroup) ((Activity) mToastVw.getContext()).findViewById(android.R.id.content);
		if (content != null) {
			ObjectAnimator.ofFloat(mToastVw, "alpha", 0.0F).setDuration(0L).start();
			content.addView(mToastVw);
			ObjectAnimator.ofFloat(mToastVw, "alpha", 0.0F, 1.0F).setDuration(167L).start();
			mToastVw.postDelayed(new Runnable() {

				@Override
				public void run() {
					ObjectAnimator animator = ObjectAnimator.ofFloat(mToastVw, "alpha", 1.0F, 0.0F);
